...
export default function AdminServiceDetailPage() {
  ...
  // Media: slot counts untuk thumbnail (1–3) dan gallery (jumlah kotak kosong)
  const [thumbnailSlotCount, setThumbnailSlotCount] = useState(1);
  const [galleryEmptySlots, setGalleryEmptySlots] = useState(1);
  const [mediaUploading, setMediaUploading] = useState<"icon" | "thumbnail" | "gallery" | null>(null);
  const [mediaError, setMediaError] = useState("");

  // Form: tambah spec value (per spec)
  const [valueFormBySpec, setValueFormBySpec] = useState<Record<number, { value: string; additional_price: string }>>({});
  const [valueSubmittingBySpec, setValueSubmittingBySpec] = useState<Record<number, boolean>>({});
  const [valueErrorBySpec, setValueErrorBySpec] = useState<Record<number, string>>({});

  // Edit spec value
  const [editingValueId, setEditingValueId] = useState<{ specId: number; valueId: number } | null>(null);
  const [editValueForm, setEditValueForm] = useState({ value: "", additional_price: "" });
  ...
  const thumbnails = service?.media?.filter((m) => m.type === "thumbnail") ?? [];
  const galleryMedia = service?.media?.filter((m) => m.type === "gallery") ?? [];
  const iconMedia = service?.media?.find((m) => m.type === "icon");

  useEffect(() => {
    if (service && thumbnails.length > 0) {
      setThumbnailSlotCount((prev) => Math.max(prev, Math.min(3, thumbnails.length)));
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      window.location.href = `/admin/dashboard/services?category=${service.category_id}`;
    } catch {
      setActioningId(null);
    }
  };
  ...
  const handleAddSpec = async (e: React.FormEvent) => {
    ...
    try {
      const options = specForm.input_type === "select" && specForm.options.trim()
        ? specForm.options.split(",").map((s) => s.trim()).filter(Boolean)
        : undefined;
      ...
      setSpecForm({ name: "", input_type: "select", options: "", is_required: true });
      await fetchService();
    } catch (e) {
      setSpecError(e instanceof Error ? e.message : "Gagal menambah spesifikasi.");
    } finally {
      ...
      await createServiceSpecificationValue(
        {
          ...
          value: valueToSend,
          additional_price: form.additional_price === "" ? 0 : Number(form.additional_price),
        },
        token
      );
      setValueFormBySpec((prev) => ({ ...prev, [spec.id]: { value: "", additional_price: "" } }));
      await fetchService();
    } catch (e) {
    ...
    try {
      await updateServiceSpecificationValue(
        ...
        editingValueId.valueId,
        {
          ...
          additional_price: editValueForm.additional_price === "" ? 0 : Number(editValueForm.additional_price),
        },
        token
      );
  ...
  if (error || !service) {
    return (
      <div className="min-h-screen flex flex-col bg-[#f5f0eb] px-4 py-8">
        <Link href="/admin/dashboard/services" className="text-sm font-semibold text-stone-600 hover:text-stone-900 mb-4">
          ← Daftar Service
        </Link>
        <p className="font-monterat-tipis text-red-700">{error ?? "Service tidak ditemukan."}</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex flex-col bg-[#f5f0eb]">
      ...
      <main className="flex-1 w-full max-w-[900px] mx-auto px-4 py-6">
        <div className="flex items-center gap-4 mb-6">
          <Link
            href={`/admin/dashboard/services?category=${service.category_id}`}
        ...
        <div className="rounded-2xl bg-white border border-stone-100 shadow-lg shadow-stone-200/30 overflow-hidden mb-6">
          <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="font-monterat-tipis text-[15px] sm:text-sm text-stone-700 leading-relaxed mt-0.5">/{service.slug}</p>
              <p className="font-monterat-tipis text-[15px] sm:text-sm text-stone-700 mt-1 leading-relaxed">
                Harga dasar: Rp {Number(service.base_price).toLocaleString("id-ID")}
              </p>
              <div className="flex flex-wrap items-center gap-2 mt-2">
                <span
                  className={`font-monterat-tipis text-xs font-semibold px-2.5 py-0.5 rounded-full ${
                    service.is_active ? "bg-emerald-100 text-emerald-800" : "bg-stone-200 text-stone-600"
                  }`}
                >
                ...
                <button
                  ...
                  className="border border-stone-200 rounded-lg px-3 py-2 hover:bg-stone-50 font-monterat-tipis text-[13px] sm:text-xs font-semibold text-stone-700 hover:text-stone-900 disabled:opacity-60 py-1.5 min-h-[44px] sm:min-h-0 flex items-center"
                >
                  {actioningId === "status" ? "..." : service.is_active ? "Nonaktifkan" : "Aktifkan"}
                </button>
                <button
                  type="button"
                  onClick={handleDeleteClick}
                  disabled={actioningId === "delete"}
                  className="border border-red-200 rounded-lg px-3 py-2 hover:bg-red-50 font-monterat-tipis text-[13px] sm:text-xs font-semibold text-red-600 hover:text-red-700 disabled:opacity-60 py-1.5 min-h-[44px] sm:min-h-0 flex items-center"
                >
          ...
          {service.description && (
            <div className="px-4 sm:px-5 pb-4 sm:pb-5">
              <p className="font-monterat-tipis text-[15px] sm:text-sm text-stone-700 leading-relaxed">{service.description}</p>
            </div>
          )}
        </div>
        ...
        <section className="rounded-2xl bg-white border border-stone-100 shadow-lg shadow-stone-200/30 overflow-hidden mb-6">
          <div className="px-4 sm:px-5 py-4 border-b border-stone-100">
            ...
            <p className="font-monterat-tipis text-[15px] sm:text-sm text-stone-700 mt-0.5 leading-relaxed">
              Tambah field spesifikasi (select, boolean, text, number) untuk layanan ini
            </p>
          </div>
          <div className="p-4 sm:p-5 space-y-4">
            <form onSubmit={handleAddSpec} className="flex flex-wrap gap-3 items-end p-4 rounded-xl bg-stone-50/80 border border-stone-100">
              <div className="flex-1 min-w-0 sm:min-w-[140px] w-full">
                <label className="font-monterat-tipis block text-[13px] sm:text-xs font-semibold text-stone-700 mb-1">Nama</label>
                <input
                  type="text"
                  value={specForm.name}
              ...
              <div className="w-full sm:w-32">
                <label className="font-monterat-tipis block text-[13px] sm:text-xs font-semibold text-stone-700 mb-1">Tipe</label>
                <select
                  value={specForm.input_type}
                  onChange={(e) =>
                    setSpecForm((p) => ({
                      ...p,
                      input_type: e.target.value as "select" | "boolean" | "text" | "number",
                    }))
                  }
                  className="font-monterat-tipis w-full min-h-[44px] sm:min-h-[40px] rounded-lg border border-stone-200 px-3 text-[15px] sm:text-sm"
                >
              ...
              {specForm.input_type === "select" && (
                <div className="flex-1 min-w-0 sm:min-w-[160px] w-full">
                  <label className="font-monterat-tipis block text-[13px] sm:text-xs font-semibold text-stone-700 mb-1">Opsi (pisah koma)</label>
                  <input
                    type="text"
                    value={specForm.options}
                    onChange={(e) => setSpecForm((p) => ({ ...p, options: e.target.value }))}
              ...
              <label className="font-monterat-tipis flex items-center gap-2 text-[15px] sm:text-sm min-h-[44px] sm:min-h-0">
                <input
                  ...
                  onChange={(e) => setSpecForm((p) => ({ ...p, is_required: e.target.checked }))}
                />
                Wajib
              </label>
              <button
            ...
            {specError && (
              <p className="font-monterat-tipis text-sm text-red-600">{specError}</p>
            )}
            ...
            {service.spesification && service.spesification.length > 0 ? (
              <ul className="space-y-3">
                {service.spesification.map((spec) => (
                  ...
                  >
                    {/* Baris utama: kiri = nama + type + status, kanan = tombol */}
                    <div className="flex flex-row items-start justify-between gap-4 flex-wrap sm:flex-nowrap">
                      {/* Kiri: nama spesifikasi, bawahnya type + wajib/opsional, disamping status */}
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span
                            className={`font-monterat-tipis text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${
                              spec.is_active ? "bg-emerald-100 text-emerald-800" : "bg-stone-200 text-stone-600"
                            }`}
                          >
                      ...
                      {/* Kanan: tombol Nonaktif, Jadikan opsional, Hapus */}
                      <div className="flex flex-wrap gap-2 order-2 flex-shrink-0">
                        ...
                        <button
                          ...
                          className="font-monterat-tipis text-xs font-semibold text-stone-700 bg-white border border-stone-200 rounded-lg px-3 py-2 hover:bg-stone-50 disabled:opacity-60"
                        >
                          {spec.is_required ? "Jadikan opsional" : "Jadikan wajib"}
                        </button>
                        <button
                          type="button"
                    ...
                    {/* Nilai spec hanya untuk tipe select & boolean */}
                    {(spec.input_type === "select" || spec.input_type === "boolean") && (() => {
                      const isBoolean = spec.input_type === "boolean";
                      const existingValues = spec.spesification_value ?? [];
                      const hasBooleanValue = isBoolean && existingValues.length > 0;
                      const selectOptions: string[] = isBoolean
                        ? []
                        : (Array.isArray(spec.options)
                          ? (spec.options as unknown[]).map((o) => (typeof o === "string" ? o : (o as { value?: string; label?: string })?.value ?? (o as { value?: string; label?: string })?.label ?? String(o)))
                          : typeof spec.options === "string"
                            ? (() => { try { const arr = JSON.parse(spec.options as string); return Array.isArray(arr) ? arr.map((x: unknown) => String(x)) : []; } catch { return []; } })()
                            : []);
                      const usedSelectValues = (spec.spesification_value ?? []).map((v) => v.value);
                      const availableSelectOptions = selectOptions.filter((opt) => !usedSelectValues.includes(opt));
                      const canAddSelectValue = spec.input_type === "select" && availableSelectOptions.length > 0;
                      const showAddForm = isBoolean ? !hasBooleanValue : canAddSelectValue;
                      return (
                      <div className="w-full mt-4 pt-3 border-t border-stone-200 space-y-2">
                        {/* Form tambah: select = dropdown (hanya opsi yang belum punya nilai) + harga + tombol, maks sesuai jumlah opsi */}
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setValueFormBySpec((p) => ({
                                  ...p,
                                  [spec.id]: {
                                    ...(p[spec.id] ?? { value: "", additional_price: "" }),
                            ...
                            <>
                              <div className="flex-1 min-w-[120px] sm:min-w-[160px]">
                                <input
                                  ...
                                  min={0}
                                  step={0.01}
                                  value={valueFormBySpec[spec.id]?.additional_price ?? ""}
                                  onChange={(e) =>
                                    setValueFormBySpec((p) => ({
                                      ...p,
                                      [spec.id]: {
                              ...
                              >
                                {valueSubmittingBySpec[spec.id] ? "..." : "Tambah nilai"}
                              </button>
                            </>
                          )}
                        </div>
                        {spec.input_type === "select" && selectOptions.length > 0 && existingValues.length >= selectOptions.length && (
                          <p className="font-monterat-tipis text-xs text-stone-500">
                            Semua opsi sudah punya nilai (maks {selectOptions.length}).
                          </p>
                        )}
                        {valueErrorBySpec[spec.id] && (
                          <p className="font-monterat-tipis text-xs text-red-600">{valueErrorBySpec[spec.id]}</p>
                        )}
                        {(spec.spesification_value ?? []).map((v) => (
                          <div key={v.id} className="flex flex-row items-center justify-between gap-2 text-sm">
                            {editingValueId?.specId === spec.id && editingValueId?.valueId === v.id ? (
                              <div className="flex flex-wrap gap-2 items-center flex-1">
                                {spec.input_type === "select" && (
                                  <select
                                    value={editValueForm.value}
                                    onChange={(e) => setEditValueForm((p) => ({ ...p, value: e.target.value }))}
                                    className="font-monterat-tipis min-h-[32px] rounded-lg border border-stone-200 px-2 text-sm min-w-[90px]"
                                  >
                                    {editValueForm.value && !selectOptions.includes(editValueForm.value) && (
                                      <option value={editValueForm.value}>{editValueForm.value}</option>
                                    )}
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditValueForm((p) => ({ ...p, additional_price: e.target.value }))
                                  }
                                ...
                                <button
                                  ...
                                  disabled={actioningId === `edit-value-${v.id}`}
                              ...
                              <>
                                <span className="font-monterat-tipis text-stone-700">
                                  {spec.input_type === "boolean" ? (
                                    <>Rp {Number(v.additional_price).toLocaleString("id-ID")}</>
                                  ) : (
                                    <>{v.value} — Rp {Number(v.additional_price).toLocaleString("id-ID")}</>
                                  )}
                                </span>
                                <button
                                  type="button"
                                  onClick={() => {
                                    setEditingValueId({ specId: spec.id, valueId: v.id });
                                    setEditValueForm({
                                      value: v.value,
              ...
              <p className="font-monterat-tipis text-sm text-stone-700">Belum ada spesifikasi.</p>
            )}
          </div>
        </section>

        {/* Media: Icon, Thumbnail, Gallery */}
        <section className="rounded-2xl bg-white border border-stone-100 shadow-lg shadow-stone-200/30 overflow-hidden mb-6">
          <div className="px-4 sm:px-5 py-4 border-b border-stone-100">
            <h2 className="font-barlow-bold text-base sm:text-lg font-bold text-stone-900">
              Media
            ...
            <p className="font-monterat-tipis text-[15px] sm:text-sm text-stone-700 mt-0.5 leading-relaxed">
              Icon (1), Thumbnail (maks 3), dan Gallery. Pilih gambar lalu otomatis ter-upload.
            </p>
          </div>
          <div className="p-4 sm:p-5 space-y-8">
            ...
            {/* ——— Icon (hanya 1) ——— */}
            <div>
              <h3 className="font-barlow-bold text-[15px] sm:text-[15px] font-semibold text-stone-800 mb-2">
                Icon
              </h3>
              <p className="font-monterat-tipis text-[15px] sm:text-[13px] text-stone-700 mb-3 leading-relaxed">
                Satu icon untuk service. Bisa dihapus lalu pilih lagi dan upload.
              </p>
              <div className="flex flex-wrap items-stretch gap-4">
                <div className="w-28 h-28 flex-shrink-0 rounded-2xl border-2 border-dashed border-stone-200 bg-stone-50/80 overflow-hidden flex items-center justify-center">
                  {iconMedia ? (
                    <div className="relative w-full h-full group">
                      ...
                      >
                        {actioningId === `media-${iconMedia.id}` ? "..." : "Hapus"}
                      </button>
                    </div>
                  ) : (
                ...
                <label className="flex flex-col justify-center gap-1 cursor-pointer">
                  ...
                  />
                  {mediaUploading === "icon" && (
                    <span className="font-monterat-tipis text-xs text-stone-700">Mengupload...</span>
                  )}
                </label>
              </div>
            </div>

            {/* ——— Thumbnail (maks 3 kotak) ——— */}
            <div>
              ...
              <p className="font-monterat-tipis text-[13px] text-stone-700 mb-3">
                Maksimal 3 thumbnail. Klik + untuk menambah kotak, lalu upload per kotak.
              </p>
              <div className="flex flex-wrap gap-4">
                {Array.from({ length: thumbnailSlotCount }).map((_, i) => {
                  ...
                  return (
                    <div
                      key={thumb ? thumb.id : `empty-thumb-${i}`}
                    ...
                    >
                      {thumb ? (
                        <div className="relative w-full h-full group">
                          ...
                          <button
                            type="button"
                            onClick={() => handleDeleteMedia(thumb.id)}
                            disabled={actioningId === `media-${thumb.id}`}
                            className="absolute inset-0 flex items-center justify-center bg-stone-900/60 opacity-0 group-hover:opacity-100 transition-opacity font-monterat-tipis text-xs font-semibold text-white rounded-2xl"
                          >
                            {actioningId === `media-${thumb.id}` ? "..." : "Hapus"}
                          </button>
                        </div>
                      ) : (
                        <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer p-2">
                          ...
                          {mediaUploading === "thumbnail" ? (
                            <span className="font-monterat-tipis text-xs text-stone-700">Upload...</span>
                          ) : (
                            <>
                              <svg className="w-8 h-8 text-stone-300 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14" />
                              </svg>
                              <span className="font-monterat-tipis text-[11px] text-stone-600 text-center">Upload</span>
                            </>
                          )}
                        </label>
                      )}
                ...
                {thumbnailSlotCount < 3 && (
                  <button
                    type="button"
                    onClick={() => setThumbnailSlotCount((c) => Math.min(3, c + 1))}
                  ...
                  >
                    ...
                    <span className="font-monterat-tipis text-xs font-semibold">+ Thumbnail</span>
                  </button>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <p className="font-monterat-tipis text-[15px] sm:text-[13px] text-stone-700 mb-3 leading-relaxed">
                Gambar gallery. Klik + untuk menambah kotak, lalu upload per kotak.
              </p>
              <div className="flex flex-wrap gap-4">
                {galleryMedia.map((m) => (
                  <div
                  ...
                  >
                    ...
                    {mediaUploading === "gallery" ? (
                      ...
                      <>
                        ...
                        <span className="font-monterat-tipis text-[11px] text-stone-600">Upload</span>
                      </>
                    )}
                  </label>
                ))}
                ...
                >
                  ...
                  <span className="font-monterat-tipis text-xs font-semibold">+ Gallery</span>
                </button>
              </div>
            </div>
          </div>
