...
export default function AdminServicesPage() {
  ...
  const [form, setForm] = useState<Omit<CreateServiceForm, "base_price"> & { base_price: string }>({
    category_id: 0,
    name: "",
    description: "",
    base_price: "",
  ...
  useEffect(() => {
    setForm((prev) => {
      ...
      if ((prev.category_id === 0 || Number.isNaN(prev.category_id)) && categories.length > 0) {
  ...
  const displayThumbs = useMemo(() => {
    ...
    list.forEach((svc) => {
      const mediaList = svc.media ?? (svc as { Media?: ServiceMedia[] }).Media ?? [];
      const thumbs = mediaList.filter(
        (m) => String((m as { type?: string; Type?: string }).type ?? (m as { type?: string; Type?: string }).Type ?? "").toLowerCase() === "thumbnail"
      );
      if (thumbs.length === 0) {
        thumbMap[svc.id] = [];
      } else {
  ...
  const handleFormChange = (
    ...
    setForm((prev) => {
      const next = { ...prev, [name]: name === "category_id" ? Number(value) : value };
      return next;
    });
    setFormError("");
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token) return;
    const thumbnailFiles = thumbnailSlots.filter((f): f is File => f != null && f.size > 0);
    const galleryFiles = gallerySlots.filter((f): f is File => f != null && f.size > 0);
      ...
      await createService(
        {
          ...
          base_price: form.base_price === "" ? undefined : Number(form.base_price),
        },
        {
          ...
          {selectedCategoryId == null && (
            <CategoryPicker
              ...
              onSelectCategory={(categoryId) => {
                ...
                router.push(`/admin/dashboard/services?category=${categoryId}`);
              }}
            />
          )}
              ...
              <div className="rounded-2xl bg-white border border-stone-100 shadow-lg shadow-stone-200/30 overflow-hidden">
                <div className="px-4 sm:px-5 py-4 border-b border-stone-100">
                  <h2 className="font-barlow-bold text-base sm:text-lg font-bold text-stone-900">
                    Manage Service{selectedCategory?.name ? ` — ${selectedCategory.name}` : ""}
                  </h2>
