...
export default function AdminCategoryEditPage() {
  ...
  const handleDelete = async () => {
    if (!token || !Number.isFinite(id) || !confirm("Yakin hapus kategori ini?")) return;
    setActioningId(id);
    try {
            ...
            <div className="px-5 pt-5 pb-5">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="min-w-0 flex-1 space-y-3">
                  <p className="font-barlow-bold text-stone-900 font-semibold">{name}</p>
                  {slug && (
                    <p className="font-monterat-tipis text-stone-700 text-sm">/{slug}</p>
                  )}
                  {description && (
                  ...
                  <span
                    className={`font-monterat-tipis text-xs font-semibold px-2.5 py-1 rounded-full inline-block ${
                      isActive ? "bg-emerald-100 text-emerald-800" : "bg-stone-200 text-stone-800"
                    }`}
                  >
              ...
              <div className="flex flex-wrap gap-3 mt-6 pt-6 border-t border-stone-100">
                ...
                >
                  {actioningId === id ? "..." : isActive ? "Nonaktifkan" : "Aktifkan"}
                </button>
                <button
                  type="button"
