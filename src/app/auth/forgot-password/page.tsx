...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await postForgotPassword({ user: user.trim() });
      setSuccess(res.message ?? "Kode verifikasi dikirim. Mengalihkan...");
      const at = Math.floor(Date.now() / 1000);
      const exp = res.expired != null ? `&expired=${res.expired}&at=${at}` : "";
      router.push(`/auth/forgot-password/otp?target=${encodeURIComponent(user.trim())}${exp}`);
    } catch (err) {
        ...
        <div className="bg-white rounded-[28px] shadow-xl shadow-stone-200/50 overflow-hidden border border-stone-100/80">
          ...
          <div className="px-5 pt-6">
            ...
            <p className="font-monterat-tipis text-center text-sm font-medium text-stone-600 mt-1.5">
              Masukkan email atau Phone yang terdaftar. Kami akan kirim kode OTP untuk reset password.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="px-5 pt-5 pb-2">
            ...
            <div className="group py-3.5 border-b border-stone-100">
              <label htmlFor="forgot-user" className="font-monterat-tipis block text-[13px] font-semibold text-stone-600 mb-2 group-focus-within:text-stone-800 transition-colors">
                Email atau Phone
              </label>
              <input
                id="forgot-user"
          ...
          <p className="px-5 font-monterat-italic text-center text-[11px] font-medium text-stone-500 pb-6 leading-relaxed">
            Kode OTP dikirim ke email atau WhatsApp sesuai data terdaftar.
          </p>
        </div>
      </div>
    </div>
