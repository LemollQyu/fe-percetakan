...
import { postForgotPassword } from "@/api/authentikasi/forgot-password/post";
import { postVerifyOtpForgotPassword } from "@/api/authentikasi/verify-otp-forgot-password/post";
...
export default function ForgotPasswordOtpPage() {
  ...
  const [expiredAt, setExpiredAt] = useState<number | null>(initialExpiredAt);
  const inputRefs = useRef<(HTMLInputElement | null)[]>([]);

  useEffect(() => {
    setExpiredAt(initialExpiredAt);
  }, [initialExpiredAt]);

  useEffect(() => {
    if (target) setReady(true);
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await postVerifyOtpForgotPassword({ otp: code });
      setSuccess(res.message ?? "Berhasil. Mengalihkan...");
      router.push(`/auth/forgot-password/set-password?user_id=${res.user_id}`);
    } catch (err) {
  ...
  const handleResend = async () => {
    ...
    setLoading(true);
    try {
      const res = await postForgotPassword({ user: target });
      setSuccess(res.message ?? "Kode baru telah dikirim.");
      const at = Math.floor(Date.now() / 1000);
      if (res.expired != null) setExpiredAt(at + res.expired);
        ...
        <div className="bg-white rounded-[28px] shadow-xl shadow-stone-200/50 overflow-hidden border border-stone-100/80">
          ...
          <div className="px-5 pt-6">
            ...
            <p className="font-monterat-tipis text-center text-sm font-medium text-stone-600 mt-1.5">
              Kode 6 digit telah dikirim ke email atau WhatsApp Anda
            </p>
          </div>
                ...
                <p className="font-monterat-tipis text-center text-[12px] font-semibold text-stone-600 mb-4">
                  {secondsLeft != null && secondsLeft > 0 ? (
                    <>
                      ...
                      <span className="font-barlow-bold text-stone-800">
