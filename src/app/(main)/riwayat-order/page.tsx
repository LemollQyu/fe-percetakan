...
/** Transform lowercase TS enum → Go format: "on_progress" → "On_progress" */
...
function getStatusConfig(rawStatus: string) {
  // Normalize: "On_progress" → "on_progress", "Waiting_payment" → "waiting_payment"
    ...
    <div className="rounded-2xl border border-stone-100 bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-4 pt-3 pb-3">
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          {totalPrice !== null && (
            <div className="col-span-1 rounded-xl bg-stone-50 border border-stone-100 px-2 py-1.5 flex flex-col items-center">
              <span className="text-[10px] text-stone-400 mb-0.5">Total</span>
        ...
        {specs.length > 0 && (
          <button
            ...
            className="w-full flex items-center justify-between rounded-xl border border-stone-100 bg-stone-50 px-3 py-2 text-[11px] font-medium text-stone-600 active:scale-[0.99] transition"
          >
        ...
        {/* File badge — dokumen buka tab baru, gambar preview modal */}
        {fileUrl &&
          (() => {
            const ft = order?.order_file?.file_type ?? ".pdf";
            const isImage = /\.(png|jpe?g|gif|webp|svg)$/i.test(ft);
              ...
              return (
                <>
                  <button
                    ...
                    className="mt-2 w-full flex items-center gap-1.5 rounded-xl border border-stone-100 bg-stone-50 px-3 py-1.5 active:scale-[0.99] transition text-left"
                  >
                    <svg
            ...
            return (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="mt-2 w-full flex items-center gap-1.5 rounded-xl border border-stone-100 bg-stone-50 px-3 py-1.5 active:scale-[0.99] transition"
              >
                <svg
                  className="w-3 h-3 text-stone-400 shrink-0"
                  fill="none"
...
export default function RiwayatOrderPage() {
  ...
  useEffect(() => {
    ...
    getMyOrders({
      token,
      status:
        status === "all" ? undefined : (toGoStatus(status) as OrderStatus),
      page,
      limit: 10,
    })
      .then((res) => {
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        const message =
          err instanceof Error ? err.message : "Gagal memuat riwayat order.";
        setError(message);
        setOrders([]);
        setHasMore(false);
      })
  ...
  const handleChangeStatus = (value: OrderStatus | "all") => {
    setStatus(value);
    setPage(1);
  };
  ...
  return (
    <main className="flex-1 w-full max-w-[430px] mx-auto px-4 py-6 pb-24">
      <RequireUser
        title="Riwayat Order"
        description="Silakan login atau daftar untuk melihat riwayat pesanan Anda."
      >
        {/* Header */}
        <div className="mb-4">
        ...
        <div className="mb-5">
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {STATUS_OPTIONS.map((opt) => {
              ...
              const cfg =
                opt.value !== "all" ? getStatusConfig(opt.value) : null;
              return (
                <button
                  key={opt.value}
                  type="button"
        ...
        {loading && (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="rounded-2xl border border-stone-100 bg-white h-32 animate-pulse"
              />
            ))}
          </div>
        )}
          ...
          <div className="space-y-3 mb-5">
            {orders.map((order: any) => (
              <OrderCard
                key={
                  String(order.id) +
                  String(order?.order_code?.code ?? order?.code ?? "")
                }
                order={order}
              />
            ))}
