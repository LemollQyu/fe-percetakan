...
export type CreateServiceForm = {
  ...
  description: string;
  base_price?: number;
  duration_per_unit?: number; // tambahan
};
...
export async function createService(
  ...
  if (form.duration_per_unit != null) {
    body.append("duration_per_unit", String(form.duration_per_unit)); // tambahan
  }
  if (files.icon) body.append("icon", files.icon);
  const thumbnailFiles = Array.isArray(files.thumbnail)
    ? files.thumbnail
    : [files.thumbnail];
