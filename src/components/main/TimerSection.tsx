...
type WSTimerMessage = {
  event: "timer_start" | "timer_pause" | "timer_resume" | "timer_add";
  order_code: string;
...
function OrderTimerSection({ code }: { code: string }) {
  ...
  const startTick = (remainingSeconds: number, startedAt: string | null) => {
    stopTick();
  ...
  const applyMessage = (msg: WSTimerMessage) => {
    ...
    if (msg.timer_status === "running") {
      startTick(msg.remaining_seconds, msg.started_at);
    } else {
      stopTick();
      setDisplaySeconds(msg.remaining_seconds);
    }
  };
  ...
  useEffect(() => {
    ...
    ws.onerror = () => console.error("WS timer error");
  ...
  // belum ada state WS → timer belum pernah start (status idle di BE)
  if (!timerState) {
    return (
      <div className="rounded-[20px] bg-white border border-stone-100 shadow-sm overflow-hidden">
        ...
        <div className="px-4 py-5 flex items-center justify-center gap-3">
          ...
          <span className="text-[12px] text-stone-400">
            Menunggu timer dimulai oleh admin...
          </span>
        </div>
      </div>
    );
  ...
  const secs = displaySeconds ?? timerState.remainingSeconds;
  const isLow = secs <= 60 && timerState.status === "running";
  const isDone = secs <= 0;
  ...
  const dotAnim =
    timerState.status === "running" && !isDone ? "animate-pulse" : "";
  ...
  return (
    <div className="rounded-[20px] bg-white border border-stone-100 shadow-sm overflow-hidden">
      ...
      <div className="px-4 py-5 flex flex-col items-center gap-2">
        <span
          className={`font-mono text-[40px] font-bold leading-none tracking-tight ${statusColor}`}
          style={{
            animation:
              isLow && !isDone
        ...
        {isDone && (
          <span className="text-[11px] text-red-500 font-semibold bg-red-50 rounded-xl px-3 py-1 border border-red-100">
            Estimasi waktu telah habis
          </span>
        )}

        {timerState.status === "paused" && !isDone && (
          <span className="text-[11px] text-amber-600 bg-amber-50 rounded-xl px-3 py-1 border border-amber-100 font-medium">
            Timer dijeda — menunggu dilanjutkan
          </span>
        )}

        {timerState.status === "running" && !isDone && !isLow && (
          <span className="text-[11px] text-blue-500 bg-blue-50 rounded-xl px-3 py-1 border border-blue-100 font-medium">
            Pesanan sedang dikerjakan
          </span>
        )}

        {isLow && !isDone && (
          <span className="text-[11px] text-red-500 bg-red-50 rounded-xl px-3 py-1 border border-red-100 font-semibold">
            Waktu hampir habis!
          </span>
        )}
      </div>
  ...
  return (
    <main className="flex-1 w-full max-w-[430px] mx-auto px-4 py-6 pb-28">
      ...
      <div className="space-y-3">
        ...
        {/* ── TIMER — hanya tampil kalau On_progress ── */}
        {isOnProgress && (
          <OrderTimerSection code={order.order_code?.code ?? ""} />
        )}
